...
public class EMediumMetadataUI extends JDialog {
	...
	private EMediumMetadataUIDelegate delegate;
	 ...
	 * @param d 
	 */
	public EMediumMetadataUI(JFrame frame, int pn, EMediumMetadataUIDelegate delegate, 
			EMediumShowUI documentUI) {
		super (frame,  true);
		this.frame = frame;
		this.pageNum = pn;
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent arg0) {
				EMediumMetadataUI.this.delegate.deleteObservers();
			}
			...
			public void mouseClicked(MouseEvent event) {
				...
				if (selectionPath != null) {
					...
					if (selection.getParent() == annotations) { 
						...
						annotationTextArea.setText(EMediumMetadataUI.this.delegate.getAnnotationText(pageNum, selectedAnnotation));
					} 

					if (event.getClickCount() >= 2)
						if (selection.getParent() == bookmarks && documentUI != null)
							try {
		...
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {
				if (event.getActionCommand().equals("Toggle bookmark"))
					EMediumMetadataUI.this.delegate.toggleBookmark (pageNum);
				else if (event.getActionCommand().equals("Add annotation")) 
					EMediumMetadataUI.this.delegate.addAnnotation (pageNum, annotationTextArea.getText());
				else if (event.getActionCommand().equals("Remove annotation")) 
					EMediumMetadataUI.this.delegate.removeAnnotation (pageNum, selectedAnnotation);
