...
final class ThumbnailFactory {
	...
	public Thumbnail getThumbnail (EMedium eMedia) throws IOException, NoSuchPageException {
		...
		File tbFile = new File(System.getProperty("user.dir") + File.separator + "docs" + File.separator + "thumbnails" + 
				File.separator + "thumb_" + dFile.getName());
		Viewer v = ViewerFactory.getInstance().getViewer(eMedia.getInternetType(), "swing");
		if (!tbFile.exists() || dFile.lastModified() > tbFile.lastModified()) {
			// criar thumbnail se n���o existe ou se o ficheiro ��� mais recente do que o thumbnail
			Thumbnail tb;
			if (v != null) {
				v.setDocument(dFile);
